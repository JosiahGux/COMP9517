...
def main():
    ...
    # 导入自定义模块（放在 main 内以避免子进程导入时执行顶层逻辑）
    from src import detection, classification, dataset

    # 解析命令行参数（放在 main 内，避免子进程重复解析）
    ...
    # 配置：数据集路径和类别名称
    DATA_DIR = "archive"  # 相对路径也可以，推荐使用绝对路径 Path(r"D:\COMP9517\DL\archive")
    CLASS_NAMES = ["Ants", "Bees", "Beetles", "Caterpillars", "Earthworms",
                   "Earwigs", "Grasshoppers", "Moths", "Slugs", "Snails", "Wasps", "Weevils"]  # 12类
    ...
    # 将 DATA_DIR 转为 Path 以便后续使用（可选）
    data_dir_path = Path(DATA_DIR)

    # 根据任务执行对应流程
    if args.task == 'train_detector':
        # 训练YOLOv8检测模型
        det_model = detection.train_detector(data_dir=data_dir_path, class_names=CLASS_NAMES,
                                             model_name=YOLO_MODEL, epochs=EPOCHS_DETECTOR)
    elif args.task == 'train_classifier':
        # 构建训练和验证数据集（使用标注裁剪昆虫目标）
        train_dataset = dataset.InsectDataset(data_dir=data_dir_path, split="train", img_size=IMG_SIZE, augmentation=True)
        val_dataset = dataset.InsectDataset(data_dir=data_dir_path, split="val", img_size=IMG_SIZE, augmentation=False)
        # 训练ResNet50分类模型
        ...
        eval_dataset = dataset.InsectDataset(data_dir=data_dir_path, split=eval_split, img_size=IMG_SIZE, augmentation=False)
        classification.evaluate_classifier(cls_model, eval_dataset, device=device)
        # 输出若干预测结果示例图像
        print("输出推理示例结果至 runs/inference/ 文件夹...")
        run_images = []
        infer_dir = os.path.join(DATA_DIR, eval_split) if os.path.isdir(os.path.join(DATA_DIR, eval_split)) \
                    else os.path.join(DATA_DIR, "images", eval_split)
        all_images = sorted(glob.glob(os.path.join(infer_dir, "*.*"))) if os.path.isdir(infer_dir) else []
        ...
        print("未知任务：", args.task)

if __name__ == "__main__":
    # Windows 下多进程保护（特别是打包时需要）
